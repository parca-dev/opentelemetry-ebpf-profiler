...
static EBPF_INLINE int cuda_correlation(struct pt_regs *ctx)
{
  ...
  // ARM64: Arguments: 4@[sp, 52]
  u64 sp = ctx->sp;
  err    = bpf_probe_read_user(&cudaId, sizeof(cudaId), (void *)(sp + 52));
#else
...
// uint64_t start, uint64_t end, uint32_t correlationId, uint32_t deviceId, uint32_t streamId,const
// char *kernelName
// AMD64 Arguments: 8@%rdx 8@%rcx 8@-40(%rbp) 4@%esi 8@%rax
// ARM64 Arguments: 8@x1 8@x2 8@[sp, 128] 4@x3 8@x0
...
#if defined(__aarch64__)
  // ARM64: 8@x1 8@x2 8@[sp, 128] 4@x3 8@x0
  start  = PT_REGS_PARM2(ctx); // x1
  end    = PT_REGS_PARM3(ctx); // x2
  u64 sp = ctx->sp;
  err    = bpf_probe_read_user(&correlationId, sizeof(correlationId), (void *)(sp + 128));
  if (err) {
    correlationId = 0;
  }
...
#else
  // AMD64: 8@%rdx 8@%rcx 8@-40(%rbp) 4@%esi 8@%rax
  start   = ctx->dx;
  end     = ctx->cx;
  u64 rbp = ctx->bp;
...
// uint32_t graphId)
// AMD64 Arguments: 8@%rdx 8@%rcx 8@-64(%rbp) 4@%esi 4@%eax
// ARM64 Arguments: 8@x1 8@x2 8@[sp, 104] 4@x3 4@x0
...
#if defined(__aarch64__)
  // ARM64: 8@x1 8@x2 8@[sp, 104] 4@x3 4@x0
  start  = PT_REGS_PARM2(ctx); // x1
  end    = PT_REGS_PARM3(ctx); // x2
  u64 sp = ctx->sp;
  err    = bpf_probe_read_user(&correlationId, sizeof(correlationId), (void *)(sp + 104));
  if (err) {
    correlationId = 0;
  }
  deviceId = PT_REGS_PARM4(ctx); // x3
...
#else
  // AMD64: 8@%rdx 8@%rcx 8@-64(%rbp) 4@%esi 4@%eax
  start   = ctx->dx;
  end     = ctx->cx;
  u64 rbp = ctx->bp;
