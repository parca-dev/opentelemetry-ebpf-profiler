  ...
  for (u8 b = 0; b < 16; b++) {
    ...
    for (u8 i = 0; i < GO_MAP_BUCKET_SIZE; i++) {
      ...
      if (tophash != 0 && kstr != NULL) {
        ...
        u8 klen = MIN(map_value->keys[i].len, CUSTOM_LABEL_MAX_KEY_LEN);
        ...
        u8 vlen    = MIN(map_value->values[i].len, CUSTOM_LABEL_MAX_VAL_LEN);
...
static EBPF_INLINE int go_labels(struct pt_regs *ctx)
{
  ...
  if (record->goOffsets.m_offset == 0) {
    ...
    //
    // NB: upstream returns -1 here (dropping the trace). parca deliberately
    // falls through to send_trace instead.
  } else {
    DEBUG_PRINT(
