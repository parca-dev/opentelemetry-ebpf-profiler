...
bool get_native_custom_labels(PerCPURecord *record, NativeCustomLabelsProcInfo *proc) {
  ...
  DEBUG_PRINT("cl: native custom labels count: %lu", tls.count);
...
#pragma unroll
  for (int i = 0; i < MAX_CUSTOM_LABELS; i++) {
    ...
    if (!lbl->key.buf)
      continue;
    CustomLabel *out_lbl = &record->customLabelsState.cla.labels[len];
...
exit:
  record->customLabelsState.cla.len = len;
