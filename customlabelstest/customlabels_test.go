...
func TestNativeCustomLabels(t *testing.T) {
	...
	cmd := exec.Command("cargo", "build", "--release", "--manifest-path", "./rust-crates/custom-labels-example/Cargo.toml")
	out, err := cmd.CombinedOutput()
	require.NoError(t, err, string(out))

	errCh := make(chan error, 1)

	cmd = exec.CommandContext(ctx, "./rust-crates/custom-labels-example/target/release/custom-labels-example")
	err = cmd.Start()
