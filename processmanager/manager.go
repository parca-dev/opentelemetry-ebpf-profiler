...
func (pm *ProcessManager) HandleTrace(bpfTrace *libpf.EbpfTrace, profileType *samples.TypeMetadata) *libpf.Trace {
	...
	// Frames appended above are kernel frames; the malformed-header report
	// below counts only the user frames decoded by this loop.
	kernelFramesLen := len(trace.Frames)

	userFrameData := bpfTrace.FrameData[numKernelFrames:]
	for frames := libpf.EbpfFrame(userFrameData); len(frames) > 0; {
		...
		if fl == 0 || fl > len(frames) {
			...
			log.Errorf("bogus trace: malformed frame header for PID %d: "+
				...
				pid, fl, len(frames), len(userFrameData),
