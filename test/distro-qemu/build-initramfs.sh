...
# host (missing libcupti), so copy it explicitly.
LIBSTDCXX=$(find /lib* /usr/lib* -name 'libstdc++.so.6' -type f -o -name 'libstdc++.so.6' -type l 2>/dev/null | head -1)
if [ -n "$LIBSTDCXX" ] && [ ! -f "$ROOTFS_DIR$LIBSTDCXX" ]; then
    real=$(readlink -f "$LIBSTDCXX")
    install -Dm755 "$real" "$ROOTFS_DIR$real"
    if [ "$real" != "$LIBSTDCXX" ]; then
        mkdir -p "$ROOTFS_DIR$(dirname "$LIBSTDCXX")"
        ln -sf "$real" "$ROOTFS_DIR$LIBSTDCXX"
    fi
fi
