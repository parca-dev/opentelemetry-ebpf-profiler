...
func (a *armExtractor) callExists(b []byte, addr, callAddr int64) (bool, error) {
	var ip int64
	for ; len(b) > 0; b = b[4:] {
		ip += 4
...
// This function gets the glref from the first line and the
// the cur_L offset from the last line.
...
// lj_dispatch_init is the first non-inlined call in luaopen_jit.
	...
	for len(b) > 0 {
		...
		if i.Op == arm64asm.LDR {
			...
			if ok {
				...
				if imm == 0x10 {
					...
					// Skip over 32 bit aligned load of sztraces
					if imm&0x7 == 0 {
		...
		if i.Op == arm64asm.ADD {
			...
			if ok && arm64asm.Reg(a1) == reg {
				a2, ok := i.Args[2].(arm64asm.ImmShift)
				if ok {
					imm := reflect.ValueOf(a2).FieldByName("imm")
					G2JOffset = imm.Uint()
				}
			}
		}
