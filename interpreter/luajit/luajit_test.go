...
func validateTrace(t *testing.T, trc *tracer.Tracer, trace *host.Trace,
	...
	lastLuaFrame := -1
	lastFramePTAddr := uint64(0)
	zeroPTAddrIndex := -1
	for i, f := range trace.Frames {
		if f.Type == libpf.LuaJITFrame {
			lastLuaFrame = i
			lastFramePTAddr = uint64(f.File)
			if lastFramePTAddr == 0 {
				zeroPTAddrIndex = i
			}
		}
	}
	...
	if zeroPTAddrIndex != lastLuaFrame {
		if lastLuaFrame > 0 {
			t.Log("lastLuaFrame: ", trace.Frames[lastLuaFrame-1])
		}
		t.Log("lastLuaFrame: ", trace.Frames[lastLuaFrame])
		t.Log("lastLuaFrame: ", trace.Frames[lastLuaFrame+1])
		if zeroPTAddrIndex != -1 {
			t.Log("zeroPTAddrIndex: ", trace.Frames[zeroPTAddrIndex])
		}
		return false
	}

	// 0 is the end of frames marker
	require.Zero(t, lastFramePTAddr)
...
func (f mockReporter) FrameMetadata(id libpf.FileID, pc libpf.AddressOrLineno,
	_ util.SourceLineno, _ uint32, funcName, fileName string) {
	key := luaKey{id, uint32(pc)}
	f.symbols[key] = luaSym{funcName, 0, fileName}
}
