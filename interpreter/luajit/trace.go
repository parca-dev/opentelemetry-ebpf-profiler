...
type trace struct {
	...
	_       uint32 /* mcloop */
	// TODONE: For arm is LJ_ABI_PAUTH defined?
...
func hashTraces(tracesAddr libpf.Address, rm remotememory.RemoteMemory) (
...
func loadTraces(tracesAddr libpf.Address, rm remotememory.RemoteMemory) (uint64, traceMap, error) {
	h, sztrace, traceAddrs, err := hashTraces(tracesAddr, rm)
	if err != nil {
		return 0, nil, err
	}
	traces := traceMap{}
