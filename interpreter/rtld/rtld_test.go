...
func TestLdsoRegexp(t *testing.T) {
	testCases := []struct {
		...
		{"/lib/ld-linux-x86-64.so.2", true},
		{"/lib64/ld-linux-x86-64.so.2", true},
		{"/usr/lib/x86_64-linux-gnu/ld-2.31.so", true},
		{"/usr/lib64/ld-linux-x86-64.so.2", true},
		...
		{"/lib/libc.so.6", false},
		{"/usr/bin/ld", false},
		{"/lib/ld-linux.so", false},
		{"/lib/x86_64-linux-gnu/ld-2.31.so", false}, // missing /usr prefix
		{"ld-linux-x86-64.so.2", false},             // missing leading /
	}

	for _, tc := range testCases {
		t.Run(tc.path, func(t *testing.T) {
			result := rtld.LdsoRegexp.MatchString(tc.path)
			require.Equal(t, tc.expected, result, "Path: %s", tc.path)
		})
	}
}
