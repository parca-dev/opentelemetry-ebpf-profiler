...
// LdsoRegexp matches ld.so paths across different distributions
var LdsoRegexp = regexp.MustCompile(
	`^(?:/(?:lib|lib64|usr/lib|usr/lib64)/` +
		`(?:ld(?:-linux)?(?:-x86-64)?\.so\.\d+|ld\.so\.\d+|ld-musl-[^/]+\.so\.\d+)|` +
		`/usr/lib(?:64)?/x86_64-linux-gnu/` +
		`(?:ld-\d+\.\d+\.so|ld(?:-linux)?(?:-x86-64)?\.so\.\d+))$`)
...
func Loader(ebpf interpreter.EbpfHandler, info *interpreter.LoaderInfo) (interpreter.Data, error) {
	// Check if this is ld.so by examining the filename with robust regexp
	fileName := info.FileName()
	if !LdsoRegexp.MatchString(fileName) {
		return nil, nil
	}
