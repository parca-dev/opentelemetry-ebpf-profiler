...
func (i *v8Instance) readCode(taggedPtr libpf.Address, cookie uint32, sfi *v8SFI) (*v8Code, error) {
	...
	if sfi == nil {
		...
		if vms.DeoptimizationDataIndex.SharedFunctionInfoWrapper != 0 {
			...
			err := i.rm.Read(sfiWrapperPtr, sfiWrapper)
			if err != nil {
...
func (d *v8Data) readIntrospectionData(ef *pfelf.File, syms libpf.SymbolFinder) error {
	...
	if vms.DeoptimizationDataIndex.SharedFunctionInfo == 0 && vms.DeoptimizationDataIndex.SharedFunctionInfoWrapper != 0 {
	...
	if vms.SharedFunctionInfo.FunctionData == 0 {
		...
		// but Node doesn't turn that on, so we can probably get away with just hardcoding it for now.
