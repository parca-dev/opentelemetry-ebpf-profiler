...
// getRegNameToID returns the appropriate register map based on runtime architecture
func getRegNameToID() map[string]usdt.Register {
	switch runtime.GOARCH {
	case "amd64":
		return x86_64RegNameToID
	case "arm64":
		return arm64RegNameToID
	default:
...
var (
	// Memory dereference with offset: -4@-1204(%rbp) or -4f@-1204(%rbp)
	...
	// Register value: 8@%rax or -4@%edi or -4f@%edi
...
func ParseUSDTArgSpec(argStr string) (*usdt.ArgSpec, error) {
	...
	// Try memory dereference with offset first
	if matches := regexRegDerefWithOffset.FindStringSubmatch(argStr); matches != nil {
		...
		regID, ok := getRegNameToID()[regName]
		if !ok {
...
// ParseUSDTArguments parses the full USDT argument string and returns a usdt.Spec
